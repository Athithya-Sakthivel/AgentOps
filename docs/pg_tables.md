...
|---|-------|----------------|------------|------------|---------|
| 1 | `users` | `src/offline/simulate_company/setup_postgres.py` | Seed script (one‑time) | Seed script | Customer profiles (business data) |
...
| Creator | Tables | When |
|---------|--------|------|
| **Seed script** (`src/offline/simulate_company/setup_postgres.py`) | `users`, `products`, `orders`, `billing`, `tickets` (schema + initial rows) | One‑time, before system starts |
