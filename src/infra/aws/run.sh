...
export TF_VAR_db_password="${TF_VAR_db_password:-SecurePass123!}"
...
Modes:
  ...
  --destroy|--delete : init backend, force_cleanup, then tofu destroy (requires --yes-delete)
  --validate         : init backend and validate backend / prereqs
  --find-version     : list remote state object versions
  --rollback-state   : restore specified state object version (requires --yes-delete)
...
apply_plan_auto() {
  # Remove stale plan file to force fresh planning
  rm -f "$PLAN_FILE"
  build_plan
...
# ----------------------------------------------------------------------
# force_cleanup: manual deletion of ECS/ASG resources (workaround for AWS provider bug)
# Also terminates orphaned EC2 instances.
# ----------------------------------------------------------------------
force_cleanup() {
  ...
  # Terminate any orphaned EC2 instances that were part of the cluster
  INSTANCE_IDS=$(aws ec2 describe-instances --filters "Name=tag:Name,Values=${name_prefix}-cluster-instance" "Name=instance-state-name,Values=running" --query "Reservations[*].Instances[*].InstanceId" --output text 2>/dev/null)
  if [ -n "$INSTANCE_IDS" ]; then
    log "Terminating orphaned EC2 instances: $INSTANCE_IDS"
    aws ec2 terminate-instances --instance-ids $INSTANCE_IDS --region "$TF_VAR_region" 2>/dev/null || true
    sleep 10
  fi
  ...
  # Detach and delete Internet Gateway (if still attached to VPC)
  local vpc_id
...
list_state_versions() {
  ...
  local key="$2"
  local json
  if ! json="$(aws s3api list-object-versions --bucket "$bucket" --prefix "$key" --output json 2>/dev/null)"; then
...
case "$MODE" in
  ...
  --destroy)
    ...
    init_backend
    force_cleanup
    # Refresh state to remove manually deleted resources
    exec_and_log "tofu-refresh-state" bash -c "cd \"$STACK_DIR\" && tofu apply -refresh-only -auto-approve"
    # Now destroy everything else (RDS, S3, VPC, etc.)
    exec_and_log "tofu-destroy" bash -c "cd \"$STACK_DIR\" && tofu destroy -input=false -auto-approve"
    log "Full destroy completed."
    ;;
  --validate)
    init_backend
